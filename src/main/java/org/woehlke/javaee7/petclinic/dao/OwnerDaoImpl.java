...
/**
 * Created with IntelliJ IDEA. User: tw Date: 06.01.14 Time: 09:38 To change
...
public class OwnerDaoImpl implements OwnerDao {
	...
	public List<Owner> getAll() {
		TypedQuery<Owner> q = entityManager.createQuery("select o from Owner o order by o.lastName,o.firstName",
				Owner.class);
	...
	public List<Owner> search(String searchterm) {
		...
		QueryBuilder qb = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Owner.class).get();
		org.apache.lucene.search.Query query = qb.keyword()
				.onFields("firstName", "lastName", "city", "pets.name", "email").matching(searchterm).createQuery();
		// wrap Lucene query in a javax.persistence.Query
		javax.persistence.Query persistenceQuery = fullTextEntityManager.createFullTextQuery(query, Owner.class);
		// execute search
